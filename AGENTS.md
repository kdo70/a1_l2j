...
- **У оружия нет SA-копий, и у него тоже есть лестница грейдов.** 434 копии вида
  `Arcana Mace - Acumen` удалены (список — `tools/weapons/retired_sa.csv`, скрипт —
  `remove_sa.ps1`, он же чистит байлисты, иконки, мультиселлы SA и ссылки на них);
  остались 4700 и 4900, потому что их носят NPC, а **оружие NPC не трогается**.
  Оставшимся 566 оружиям отчеканены 1772 копии (`12000..13771`) — по одной на каждый
  грейд выше своего, с верхом грейда по своему классу оружия; ни один существующий
  предмет при этом не изменён. Источник — `tools/weapons/weapons.csv`, генератор —
...
- **Свечение заточенного оружия растёт со ступенью заточки** (+4/+7/+10/+12/+14/+15/+17)
  и берётся из `EnchantGlow.u`, который кладётся в `system\` клиента. Вид оружия пишется
