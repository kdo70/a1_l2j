...
- **Дроп-лист виден игроку по shift+click** по монстру или РБ, если это включено в
  ...
  за убийство.
- **Чемпионы бывают двух видов — красные и синие.** Настройки —
  `config/mods/championmobs.properties`, у каждого вида свой блок ключей
  (`ChampionMobsRed*` / `ChampionMobsBlue*`): частота, диапазон уровней,
  множители характеристик, отдельные множители Xp и Sp, адены, дропа и спойла,
  титул, цвет надписи и собственный дроп. Разбор живёт в
  `model/ChampionSettings.java`, вид — `enums/ChampionType.java`, состояние
  монстра — `Monster.getChampionType()` / `getChampionSettings()`.
  Расписание у каждого вида своё — `ChampionMobs<вид>Schedule`, список окон
  `дни-часОт:часДо` через `;` (`1-5-0:24` — будни, `6,7-0:24` — выходные,
  `22:2` — через полночь). Пустое расписание = всегда. Проверка идёт в момент
  спавна.
  Аура берётся из `TeamType` вида, цвет имени и титула — из `NameColor`
  (нужен пропатченный `engine.dll`, см. `docs/npc-name-colors.md`), пустой
  `Title` оставляет титул из датапака.
  Рейты дропа и спойла (по умолчанию x2) домножаются к серверным; множитель
  живёт в `Monster.getChampionRateMultiplier` и берётся оттуда и движком дропа,
  и окном дроп-листа. Собственный дроп чемпиона (`ChampionMobs<вид>Drop`,
  `idItem:min-max:chance;...`) разыгрывается построчно, со штрафом за перекач и
  без серверных рейтов, и виден в shift+click отдельной верхней группой
  «Чемпион» (подпись — `champion` в `data/xml/droplist.xml`).
- **`//set hero` выдаёт геройство насовсем и сразу активным.** В отличие от стокового `//sethero`,
  который зажигает флаг до ближайшего релога, эта команда пишет таблицу `heroes` через
  `HeroManager.grantHero`. **Ставить `active = 1` обязательно:** `PcInventory.equipItem` пускает
  геройское оружие по `HeroManager.isActiveHero()`, а не по `Player.isHero()`, поэтому герой со
  статусом «played, но не активирован» носит ауру и не может надеть Infinity Blade.
  Команда **переключает**, как соседняя `//set noble`: второй раз забирает геройство обратно и
  снимает надетое геройское. `//set hero 0` / `//set hero 1` говорят прямо, без угадывания.
  Ауру героя у GM показывает ещё и `Config.GM_HERO_AURA`, так что аура — не доказательство того,
  что команда сработала.
- **`//enchant` без аргументов открывает панель** (`data/html/admin/enchant.htm`): в поле вводится
  уровень заточки, потом жмётся слот, и панель открывается снова — заточить весь комплект можно не
  перенабирая уровень. Старый вид `//enchant <слот> <уровень>` работает как работал; панель просто
  подставляет `$enchant` в тот же bypass.
- **Книга рейдбоссов** — окно со списком РБ, которых персонаж уже бил, с уровнями охоты
  ...
  `data/xml/raidbook.xml`, устройство — `docs/raidboss-book.md`. **Открывается разговором
  с Adventure Guildsman** (любой NPC с `type="Adventurer"`, например 31805) —
  `Adventurer.showChatWindow` зовёт `RaidBookManager.show`, никакого предмета и никаких
  промежуточных диалогов у этого типа NPC не осталось. Босс попадает в книгу первым ударом
  по нему; пустая книга пишет подпись `hint` вместо списка.
...
- **У оружия нет SA-копий, нет shadow-оружия, и у него тоже есть лестница грейдов.**
  443 копии вида `Arcana Mace - Acumen` удалены (список — `tools/weapons/retired_sa.csv`,
  скрипт — `remove_sa.ps1`, он же чистит байлисты, иконки, мультиселлы SA и ссылки на них).
  Две последние — 4700 и 4900 — держали в руках NPC, поэтому их убирал `remove_weapons.ps1`
  по тому же списку: он переставил руку на базовое оружие (76 и 210), у которого тот же меш и
  та же текстура, так что в мире это не видно. **Оружие NPC не трогается ни при каких условиях**
  кроме этого одного места — правило нарушалось и было восстановлено `restore_weapons.ps1`.
  Ещё 49 оружий (дуалы уцелевших мечей, `7887`, `240`) и 115 `Shadow Item: ...` выведены
  из игры целиком — `retired_weapons.csv` и `retired_shadow.csv`, скрипт `remove_weapons.ps1`;
  вместе с shadow-оружием ушла и фича обмена купонов (`ShadowWeapon.java`, её регистрация
  в `scripts.xml`, её HTM и 90 ссылок у вилладж-мастеров).
  **No Grade оружие не прокачивается**, всё, что выше, доходит до S: 461 оружие, 989 копий
  (`12000..12988`) — по одной на каждый грейд выше своего, с верхом грейда по своему классу
  оружия; ни один существующий предмет при этом не изменён.
  **Оружие, неотличимое в мире, слито в одно** (меш+текстура+тип+грейд): 106 предметов ушли,
  руки NPC переставлены на выжившего — `find_duplicates.ps1` предлагает в `duplicates.csv`,
  `dedup_weapons.ps1` применяет. **49 из них раздавали квесты, и 52 таких квеста удалены целиком** —
  включая первую и третью профессию (Saga, Trial/Testimony) и `Q234_FatesWhisper`. Побочно
  пришлось убрать требование `Q234` из `VillageMaster.checkQuests`, иначе сабкласс закрылся бы
  для всех, кроме ноблесс.
  **Запретов на оружии нет** — `free_weapons.ps1` снял продажу/обмен/склад/выброс со всего,
  кроме монстрового и двух проклятых мечей.
  **В GM-магазине список отсортирован по стартовому грейду** (в S сначала те, кого подняли с D),
  монстровое снаряжение — в своей вкладке `9136`. Байлисты оружия: `9001..9053` и `9129..9136`;
  `9105..9111` принадлежат страницам брони, лезть в них нельзя.
  **Монстровое опознаётся по иконке `weapon_monster_i00`, а не по имени** — имя промахивается мимо
  ...
  Диапазоны id разведены: броня владеет `10000..11409`, оружие — с `12000`. Устройство —
  `docs/weapon-grades.md`.
- **Свечение заточенного оружия растёт со ступенью заточки** (+4/+7/+10/+12/+14/+15/+17)
  и берётся из `EnchantGlow.u`, который кладётся в `system\` клиента **как есть** — голым,
  `licensee 0`, единственным незавёрнутым пакетом в папке: клиент такой читает, а вот
