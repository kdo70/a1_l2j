# Every armor set, in every grade

`generate.ps1` turns the 130 armor looks of the datapack into **780 sets** — each look exists as No Grade,
D, C, B, A and S — and `patch_client.ps1` teaches the client the item ids it minted.
...
| `generated/upgrade_chain.tsv` | one line per look : the six ids it wears, No Grade to S. |
...
| `origGrade` | the grade the chest had before all this. Only used to rescale a retail set bonus onto the six grades. |
| `head`, `legs`, `gloves`, `feet` | the item each slot is **cloned from**. Sharing is fine and retail does it too — Zubei heavy, light and robe all wear helmet 503. |
